...
export function getTradeV3FixtureFor(xDecimals: number, yDecimals: number, price: string) {
  return async function ([wallet]: Wallet[]) {
    const { oracle } = await getOracleV3FixtureFor(xDecimals, yDecimals)([wallet])
