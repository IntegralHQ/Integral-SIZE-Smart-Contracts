...
describe('TwapPair.getSwapAmount1Out', () => {
  ...
  it('returns the number of token1 swapped for token0', async () => {
    ...
    const amount1Out = await pair.getSwapAmount1Out(amount0In, priceInfo)
  ...
  it('returns correct values', async () => {
    const { addLiquidity, token0, pair, wallet, getState, setupUniswapPair } = await loadFixture(pairFixture)
    ...
    const expectedOutputAmount = expandTo18Decimals('378.41135')

    expect(await pair.getSwapAmount1Out(inputAmount, priceInfo)).to.eq(expectedOutputAmount)
    ...
    const expectedOutputAmount2 = expandTo18Decimals('378.41135')

    expect(await pair.getSwapAmount1Out(inputAmount2, priceInfo)).to.eq(expectedOutputAmount2)

    const after = await getState()
    expect(after.reserves[0]).to.eq(expandTo18Decimals(100.997))
    expect(after.reserves[1]).to.eq(expandTo18Decimals('1758.58865'))
  })
})
