...
describe('TwapPair.getSwapAmount0Out', () => {
  ...
  it('returns correct values', async () => {
    ...
    const expectedOutputAmount = expandTo18Decimals('0.262679488868396786')

    expect(await pair.getSwapAmount0Out(inputAmount, priceInfo)).to.eq(expectedOutputAmount)
    ...
    const expectedOutputAmount2 = expandTo18Decimals('0.262679488868396786')

    expect(await pair.getSwapAmount0Out(inputAmount2, priceInfo)).to.eq(expectedOutputAmount2)

    const after = await getState()
    expect(after.reserves[0]).to.eq(expandTo18Decimals('99.737320511131603214'))
