...
contract TokenSharesTest {
    ...
    function sharesToAmount(address token, uint256 shares, uint256 amountLimit, address refundTo) public {
        uint256 result = tokenShares.sharesToAmount(token, shares, amountLimit, refundTo);
        emit Result(result);
    }

    function amountToShares(address token, uint256 amount, bool wrap) public payable {
